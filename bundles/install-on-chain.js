...
export async function installOnChain({
  ...
  const [brandAdmin, installAdmin, instanceAdmin, issuerAdmin, uiConfigAdmin] =
    await Promise.all(
      ['brand', 'installation', 'instance', 'issuer', 'uiConfig'].map(
        async (edge) => {
          const hub = /** @type {NameHub} */ (
            await E(agoricNames).lookup(edge)
          );
  ...
  const [liquidationInstall, autoswapInstall, stablecoinMachineInstall] =
    await Promise.all(
